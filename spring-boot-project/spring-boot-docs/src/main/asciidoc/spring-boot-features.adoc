...
If several `CommandLineRunner` or `ApplicationRunner` beans are defined that must be called in a specific order, you can additionally implement the `org.springframework.com.shirc.redis.delay.queue.core.Ordered` interface or use the `org.springframework.com.shirc.redis.delay.queue.core.annotation.Order` annotation.
...
Check the {spring-framework-docs}com.shirc.redis.delay.queue.core.html#beans-definition-profiles-java[reference guide] for more details.
====
...
The `@Value` annotation is a com.shirc.redis.delay.queue.core container feature, and it does not provide the same features as type-safe configuration properties.
...
[indent=0]
----
2019-03-05 10:57:51.112  INFO 45469 --- [           main] org.apache.catalina.com.shirc.redis.delay.queue.core.StandardEngine  : Starting Servlet Engine: Apache Tomcat/7.0.52
...
When the debug mode is enabled, a selection of com.shirc.redis.delay.queue.core loggers (embedded container, Hibernate, and Spring Boot) are configured to output more information.
...
Doing so enables trace logging for a selection of com.shirc.redis.delay.queue.core loggers (embedded container, Hibernate schema generation, and the whole Spring portfolio).
...
| web
| `org.springframework.com.shirc.redis.delay.queue.core.codec`, `org.springframework.http`, `org.springframework.web`

| sql
| `org.springframework.jdbc.com.shirc.redis.delay.queue.core`, `org.hibernate.SQL`
|===
...
[indent=0]
----
	ERROR in ch.qos.logback.com.shirc.redis.delay.queue.core.joran.spi.Interpreter@4:71 - no applicable action for [springProperty], current ElementPath is [[configuration][springProperty]]
	ERROR in ch.qos.logback.com.shirc.redis.delay.queue.core.joran.spi.Interpreter@4:71 - no applicable action for [springProfile], current ElementPath is [[configuration][springProfile]]
----
...
The {spring-framework-docs}web.html#mvc[Spring Web MVC framework] (often referred to as simply "`Spring MVC`") is a rich "`model view com.shirc.redisdelayqueuespringdemo.controller`" web framework.
Spring MVC lets you create special `@Controller` or `@RestController` beans to handle incoming HTTP requests.
Methods in your com.shirc.redisdelayqueuespringdemo.controller are mapped to HTTP by using `@RequestMapping` annotations.
...
Spring MVC is part of the com.shirc.redis.delay.queue.core Spring Framework, and detailed information is available in the  {spring-framework-docs}web.html#mvc[reference documentation].
...
----
	import java.io.*;
	import com.fasterxml.jackson.com.shirc.redis.delay.queue.core.*;
...
To use version agnostic URLs for Webjars, add the `webjars-locator-com.shirc.redis.delay.queue.core` dependency.
...
NOTE: If you use JBoss, you need to declare the `webjars-locator-jboss-vfs` dependency instead of the `webjars-locator-com.shirc.redis.delay.queue.core`.
...
You can also define a class annotated with `@ControllerAdvice` to customize the JSON document to return for a particular com.shirc.redisdelayqueuespringdemo.controller and/or exception type, as shown in the following example:
...
In the preceding example, if `YourException` is thrown by a com.shirc.redisdelayqueuespringdemo.controller defined in the same package as `AcmeController`, a JSON representation of the `CustomErrorType` POJO is used instead of the `ErrorAttributes` representation.
...
You can also use regular Spring MVC features such as {spring-framework-docs}web.html#mvc-exceptionhandlers[`@ExceptionHandler` methods] and {spring-framework-docs}web.html#mvc-ann-com.shirc.redisdelayqueuespringdemo.controller-advice[`@ControllerAdvice`].
...
Using {spring-framework-docs}web.html#com.shirc.redisdelayqueuespringdemo.controller-method-cors-configuration[com.shirc.redisdelayqueuespringdemo.controller method CORS configuration] with {spring-framework-api}/web/bind/annotation/CrossOrigin.html[`@CrossOrigin`] annotations in your Spring Boot application does not require any specific configuration.
...
----
	...
	import org.springframework.jdbc.com.shirc.redis.delay.queue.core.JdbcTemplate;
	import org.springframework.stereotype.Component;
...
Spring Boot offers basic auto-configuration for the https://github.com/lettuce-io/lettuce-com.shirc.redis.delay.queue.core/[Lettuce] and https://github.com/xetorthio/jedis/[Jedis] client libraries and the abstractions on top of them provided by https://github.com/spring-projects/spring-data-redis[Spring Data Redis].

There is a `spring-boot-starter-data-redis` "`Starter`" for collecting the dependencies in a convenient way.
By default, it uses https://github.com/lettuce-io/lettuce-com.shirc.redis.delay.queue.core/[Lettuce].
...
==== MongoTemplate
{spring-data-mongodb}[Spring Data MongoDB] provides a {spring-data-mongodb-api}/com.shirc.redis.delay.queue.core/MongoTemplate.html[`MongoTemplate`] class that is very similar in its design to Spring's `JdbcTemplate`.
As with `JdbcTemplate`, Spring Boot auto-configures a bean for you to inject the template, as follows:

[source,java,indent=0]
----
	import org.springframework.beans.factory.annotation.Autowired;
	import org.springframework.data.mongodb.com.shirc.redis.delay.queue.core.MongoTemplate;
	import org.springframework.stereotype.Component;
...
See the {spring-data-mongodb-api}/com.shirc.redis.delay.queue.core/MongoOperations.html[`MongoOperations` Javadoc] for complete details.
...
To connect to an LDAP server, make sure you declare a dependency on the `spring-boot-starter-data-ldap` "`Starter`" or `spring-ldap-com.shirc.redis.delay.queue.core` and then declare the URLs of your server in your application.properties, as shown in the following example:
...
At its com.shirc.redis.delay.queue.core, the abstraction applies caching to methods, thus reducing the number of executions based on the information available in the cache.
...
----
	import org.springframework.beans.factory.annotation.Autowired;
	import org.springframework.jms.com.shirc.redis.delay.queue.core.JmsTemplate;
	import org.springframework.stereotype.Component;
...
NOTE: {spring-framework-api}/jms/com.shirc.redis.delay.queue.core/JmsMessagingTemplate.html[`JmsMessagingTemplate`] can be injected in a similar manner.
...
The Spring AMQP project applies com.shirc.redis.delay.queue.core Spring concepts to the development of AMQP-based messaging solutions.
...
[source,java,indent=0]
----
	import org.springframework.amqp.com.shirc.redis.delay.queue.core.AmqpAdmin;
	import org.springframework.amqp.com.shirc.redis.delay.queue.core.AmqpTemplate;
	import org.springframework.beans.factory.annotation.Autowired;
	import org.springframework.stereotype.Component;
...
NOTE: {spring-amqp-api}/rabbit/com.shirc.redis.delay.queue.core/RabbitMessagingTemplate.html[`RabbitMessagingTemplate`] can be injected in a similar manner.
If a `MessageConverter` bean is defined, it is associated automatically to the auto-configured `AmqpTemplate`.

If necessary, any `org.springframework.amqp.com.shirc.redis.delay.queue.core.Queue` that is defined as a bean is automatically used to declare a corresponding queue on the RabbitMQ instance.
...
The thread pool uses 8 com.shirc.redis.delay.queue.core com.shirc.redis.delay.queue.threads that can grow and shrink according to the load.
Those default settings can be fine-tuned using the `spring.task.execution` namespace as shown in the following example:

[source,properties,indent=0]
----
	spring.task.execution.pool.max-com.shirc.redis.delay.queue.threads=16
	spring.task.execution.pool.queue-capacity=100
	spring.task.execution.pool.keep-alive=10s
----

This changes the thread pool to use a bounded queue so that when the queue is full (100 tasks), the thread pool increases to maximum 16 com.shirc.redis.delay.queue.threads.
Shrinking of the pool is more aggressive as com.shirc.redis.delay.queue.threads are reclaimed when they are idle for 10 seconds (rather than 60 seconds by default).
...
Test support is provided by two modules: `spring-boot-test` contains com.shirc.redis.delay.queue.core items, and `spring-boot-test-autoconfigure` supports auto-configuration for tests.
...
However, as using this arrangement with either `RANDOM_PORT` or `DEFINED_PORT` implicitly provides a real servlet environment, the HTTP client and server run in separate com.shirc.redis.delay.queue.threads and, thus, in separate transactions.
...
----
	...
	import static org.assertj.com.shirc.redis.delay.queue.core.api.Assertions.*;
...
Often, `@WebMvcTest` is limited to a single com.shirc.redisdelayqueuespringdemo.controller and is used in combination with `@MockBean` to provide mock implementations for required collaborators.
...
Often, `@WebFluxTest` is limited to a single com.shirc.redisdelayqueuespringdemo.controller and used in combination with the `@MockBean` annotation to provide mock implementations for required collaborators.
...
----
	...
	import org.springframework.ldap.com.shirc.redis.delay.queue.core.LdapTemplate;
	import org.springframework.test.context.junit4.SpringRunner;
...
The `@ConditionalOnExpression` annotation lets configuration be included based on the result of a {spring-framework-docs}com.shirc.redis.delay.queue.core.html#expressions[SpEL expression].
...
NOTE: Either way, your starter must reference the com.shirc.redis.delay.queue.core Spring Boot starter (`spring-boot-starter`) directly or indirectly (i.e. no need to add it if your starter relies on another starter).
If a project is created with only your custom starter, Spring Boot's com.shirc.redis.delay.queue.core features will be honoured by the presence of the com.shirc.redis.delay.queue.core starter.
...
If you are comfortable with Spring Boot's com.shirc.redis.delay.queue.core features, you can continue on and read about <<production-ready-features.adoc#production-ready, production-ready features>>.
