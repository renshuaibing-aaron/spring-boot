...
When using Spring MVC or Spring Web Flux, operations that return a `org.springframework.com.shirc.redis.delay.queue.core.io.Resource` automatically support range requests.
...
[[production-ready-endpoints-custom-com.shirc.redisdelayqueuespringdemo.controller]]
...
To use Jolokia, include a dependency to `org.jolokia:jolokia-com.shirc.redis.delay.queue.core`.
...
----
	<dependency>
		...
		<artifactId>jolokia-com.shirc.redis.delay.queue.core</artifactId>
	</dependency>
----
...
=== Supported Metrics
Spring Boot registers the following com.shirc.redis.delay.queue.core metrics when applicable:
...
----
<1> A com.shirc.redisdelayqueuespringdemo.controller class to enable timings on every request handler in the com.shirc.redisdelayqueuespringdemo.controller.
...
If you need to apply customizations to specific `Meter` instances you can use the `io.micrometer.com.shirc.redis.delay.queue.core.instrument.config.MeterFilter` interface.
