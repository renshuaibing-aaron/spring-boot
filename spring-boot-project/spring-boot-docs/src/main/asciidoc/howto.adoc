...
| `com.fasterxml.jackson.com.shirc.redis.delay.queue.core.JsonGenerator.Feature`
...
| `com.fasterxml.jackson.com.shirc.redis.delay.queue.core.JsonParser.Feature`
...
The multipart support is helpful when you want to receive multipart encoded file data as a `@RequestParam`-annotated parameter of type `MultipartFile` in a Spring MVC com.shirc.redisdelayqueuespringdemo.controller handler method.
...
A `ViewResolver` is a com.shirc.redis.delay.queue.core component of Spring MVC, translating view names in `@Controller` to actual `View` implementations.
...
|YAML
| `com.fasterxml.jackson.com.shirc.redis.delay.queue.core:jackson-databind` + `com.fasterxml.jackson.dataformat:jackson-dataformat-yaml`
| `log4j2.yaml` + `log4j2.yml`

|JSON
| `com.fasterxml.jackson.com.shirc.redis.delay.queue.core:jackson-databind`
...
Spring Boot exposes a set of useful properties (from the `spring.data.rest` namespace) that customize the {spring-data-rest-api}/com.shirc.redis.delay.queue.core/config/RepositoryRestConfiguration.html[`RepositoryRestConfiguration`].
...
To automatically run Flyway database migrations on startup, add the `org.flywaydb:flyway-com.shirc.redis.delay.queue.core` to your classpath.
...
To automatically run Liquibase database migrations on startup, add the `org.liquibase:liquibase-com.shirc.redis.delay.queue.core` to your classpath.
...
See {spring-batch-api}/com.shirc.redis.delay.queue.core/configuration/annotation/EnableBatchProcessing.html[The Javadoc of `@EnableBatchProcessing`] for more details.
...
See {spring-boot-autoconfigure-module-code}/batch/BatchAutoConfiguration.java[BatchAutoConfiguration] and https://github.com/spring-projects/spring-batch/blob/master/spring-batch-com.shirc.redis.delay.queue.core/src/main/java/org/springframework/batch/com.shirc.redis.delay.queue.core/configuration/annotation/EnableBatchProcessing.java[@EnableBatchProcessing] for more details.
...
By default, the Spring Boot starter (`spring-boot-starter-data-redis`) uses https://github.com/lettuce-io/lettuce-com.shirc.redis.delay.queue.core/[Lettuce].
	...
	<dependency>
		...
		<exclusions>
			<exclusion>
				...
				<artifactId>lettuce-com.shirc.redis.delay.queue.core</artifactId>
			</exclusion>
		</exclusions>
	</dependency>
	<dependency>
